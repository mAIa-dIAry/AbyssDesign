    ...
    docs: {
      description: {
        component:
          ...
          '- Kontekstowe akcje typowe dla danej karty (np. odświeżenie danych, ustawienia widoku) umieszczaj w `header-append` jako płaskie przyciski ikonowe (`AbyssButton` + `flat` + `size="medium"` + `aria-label`, bez widocznego labela). Karta nadpisuje `--border-radius` na `12px`, żeby krzywizna złożyła się z niskim narożnikiem (`min-height: 48px`, `border-radius: 16px`) — nie bierz promienia ze skali `size="medium"` (6px) ani z wyższego headera tabeli.\n' +
          ...
          '- Formularz w `#content` używa standardowego `AbyssButton` `size="big"` — bez `flat` i bez `embedded`.\n\n' +
...
export const WithHeaderActions: Story = {
  ...
  play: async ({ canvas }) => {
    ...
    await expect(title).toBeVisible();
    const refresh = canvas.getByRole('button', { name: 'Odśwież dane' });
    await expect(refresh).toBeVisible();
    await expect(
      getComputedStyle(refresh).getPropertyValue('--border-radius').trim(),
    ).toBe('12px');
