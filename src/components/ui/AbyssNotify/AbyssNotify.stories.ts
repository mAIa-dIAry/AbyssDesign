    ...
    docs: {
      description: {
        component:
          ...
          'W `AbyssTemplateRoot` montuj `AbyssNotifyHost` w slocie `#overlay`. Host overlay ma `padding: calc(12px + env(safe-area-inset-top, 0px)) 8px 12px` i `max-height: 100%`; `overflow: auto` to przełącznik po 0,2 s ciszy — dodawanie nie zdejmuje `auto` od razu. Poza szablonem `AbyssNotifyHost` z `standalone` (klasa `abyss-notify-queue`). ' +
          'Wejście (z góry) i zejście (w dół) trwają **0,2 s**. Slot (toast + 8px odstęp jako `::after`) zwija wysokość, `overflow: visible` — toast wystaje ze slotu. Ostatni toast przy zejściu **nie zwija wysokości** — tylko `translateY` i opacity. Padding hosta (`calc(12px + env(safe-area-inset-top, 0px)) 8px 12px`) mieści `translateY` i omija pasek systemowy. Widoczność kontroluj `v-model` (`modelValue` domyślnie `true`); nowo zamontowany toast też wchodzi z `appear`. ' +
